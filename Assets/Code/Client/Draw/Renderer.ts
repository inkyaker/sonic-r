...
export class Renderer {
	...
	constructor(RootTransform: Transform, RigParent: GameObject) {
		...
		for (const [_, Object] of pairs(RigParent.GetComponentsInChildren<Transform>(true))) this.Parts.push(Object.gameObject);
	}
	...
	public Draw(DeltaTime: number, DrawInfo: DrawInformation) {
		...
		this.Angle = Quaternion.Slerp(Angle, this.Angle, (0.675 ** 60) ** DeltaTime);
	...
	public SetVisible(Visible: boolean) {
		...
		for (const [_, Mesh] of pairs(this.Parts)) Mesh.SetActive(Visible);
	}
}
