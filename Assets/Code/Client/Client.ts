...
export default class DSClient extends AirshipBehaviour {
	...
	public ToLocal(Vector: Vector3) {
		...
		// may need to swap
		return Quaternion.Inverse(MathAngle)
			.mul(Vector)
	...
	/**
	 * Stretch and compress jumpball vertically
	 * @param StretchTime time multiplier
	 */
	public StretchJumpBall(StretchTime?: number) {
		this.Flags.JumpStretchTimer = math.ceil(this.Config.JumpStretchTimer * (StretchTime ?? 1));
	}
