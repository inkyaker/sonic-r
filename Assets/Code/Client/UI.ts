...
export default class UI extends AirshipSingleton {
	...
	private UpdateHomingTarget(DeltaTime: number) {
		...
		this.Reticle.sizeDelta = new Vector2(
			this.Reticle.sizeDelta.x,
			Config.ReticleMaxDistance * Config.ReticleDistanceCurve.Evaluate(math.clamp01(this.ReticleTime / Config.ReticleTimeMax)),
		);
	...
	public SetHomingTarget(Target: Transform | undefined) {
		if (this.PlayerHomingTarget !== Target) {
			...
			if (this.CurrentSound && Target !== undefined) this.CurrentSound.Play("UI/HomingReticleSXSG.wav"); // optionally: UI/HomingReticleUnleashed.wav
		}
	}
